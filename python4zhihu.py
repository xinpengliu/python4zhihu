...
def login_zhihu(login_info):
    ...
    account_type = ''
    # 判断帐号信息是否有效
    if re.match('^1\d{10}$', user_name):  #以1开头，后面10位数字--电话号码为登录帐号。
        account_type = 'phone_num'
    elif re.match('^\S+@\d+\.\S+$', user_name):# \S匹配任何非空白字符，它相当于[^\t\n\r\f\v]
        account_type = 'email'
    else:
    ...
    # 获取_xsrf信息-- CSRF(Cross-site request forgery),中文名称:跨站请求伪造
    ...
    try:
        ret = login_session.post(login_url, data=up_form, headers=login_header)#提交登录信息post请求
